...
| **HTTPS hardware gateway (REAL ESP32 over TLS, production)** | ✅ | `POST /api/hardware/telemetry` (mounted at `/api/hardware`) — key-protected with the server-side `HARDWARE_GATEWAY_TOKEN` sent as `x-device-key`; answers 503 `GATEWAY_NOT_CONFIGURED` until the key is set, so an unconfigured host is never an open write path. No browser session on the device. Device external id must be pre-registered on its field (Sensors workspace) — registration is authoritative; conflicting payload `field_id` → 409. Canonical firmware payload (message_id/timestamp/readings) → same shared pipeline: physical-range validation, dedupe, OBSERVED evidence (provenance `HTTPS gateway`), heartbeat, SSE, world-model refresh. `GET /api/hardware/health` public probe (never leaks the key). 9 HTTP-level tests on a real server. Reference firmware: `hardware/esp32/agrifur_esp32_https/agrifur_esp32_https.ino` (WiFiClientSecure, NTP-gated TLS, optional root CA) |
